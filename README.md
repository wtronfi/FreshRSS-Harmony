...
### 首页与订阅
- 顶部9按钮快捷操作栏：展开、折叠、刷新、全部、未读、添加、搜索、最近、收藏
- Feed 订阅浏览
- 文件夹分组
- Feed / 文件夹 / 总未读数量
- 收藏数量统计
- 文件夹展开 / 折叠（支持临时展开/折叠，不改变记忆状态）
- 文件夹展开状态记忆
- 新增订阅
- 修改订阅标题
- 移动订阅
...
### 文章列表
- 顶部6按钮快捷操作栏：刷新、全部、未读、滚动已读、已加载已读、全源已读
- 分页加载文章
- 连续加载超过 50 篇文章
- 仅显示未读 / 显示全部
- FreshRSS 真实未读数量
...
- 浅色主题
- 深色主题
- 跟随系统主题（自动适配手机深色/浅色模式）
- 纯黑暗模式（仅在深色主题下生效，状态栏与导航栏同步纯黑）
- 护眼模式（全局米白背景 + 深灰文字，柔和护眼，状态栏与导航栏同步护眼配色）
- 首页布局自定义（文件夹字号、订阅源字号、左边距、行间距）
- 文章列表字号自定义
- 阅读界面字号、行距、段间距自定义
...
- 正文图片缓存
- 指定订阅源主动缓存
- 多订阅源文章列表并行拉取 + 图片统一限流下载（慢源不阻塞其他源）
- 离线启动
- 离线查看订阅
- 离线阅读已缓存文章
- 网络失败时缓存降级
- 可配置文章缓存容量、媒体缓存容量、已读文章保留时长
- 自动淘汰长期未使用缓存
- 缓存容量控制
- 缓存版本升级与旧缓存失效处理
- 存储空间保护
...
### 设置与维护
设置主界面菜单：常规、显示和外观、手势、缓存、数据维护、同步诊断、账号、关于。

包括：
- 常规设置（排序、刷新策略、文件夹显示、首页行为）
- 显示和外观（主题、护眼模式、纯黑暗模式、首页设置、文章列表设置、阅读页面设置）
- 手势设置（左右滑动动作、到顶下滑打开原文、全部已读返回）
- 缓存设置（已读文章保留时长、文章缓存容量、媒体缓存容量，均支持自定义数值输入）
- 数据维护（数据健康检查、分级清理：文章缓存/媒体缓存/阅读历史/搜索历史/同步历史/稳定性日志，支持一键清理以上所有）
- 同步诊断（同步状态、错误记录、稳定性日志导出为 TXT 文件）
- 账号设置
- 关于页面
- 首页/文章列表/正文齿轮设置按钮：短按进入设置，长按直接弹出对应设置子界面
...
```text
FreshRSS Harmony 2.18
Version Name : 2.18
Version Code : 2026090513
